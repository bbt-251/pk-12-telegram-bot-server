...
async function handleBidFlow(
    ...
    if (state?.step === 'confirm') {
        ...
        // Show confirmation
        await showBidConfirmation(chatId, state.bidAmount!, trucks, state.loadRequestId, state.displayID);
        return;
    }
        ...
        if (state.existingBidId) {
            ...
            bid = await updateBid(
                state.existingBidId,
                {
                    ...
                    numberOfTrucks: state.numberOfTrucks
                },
 ...
 */
export async function restartBidFlow(
    chatId: number
): Promise<void> {
    const pendingBid = getPendingBid(chatId);
    if (!pendingBid) {
